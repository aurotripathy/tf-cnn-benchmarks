...
flags.DEFINE_boolean(
    'xla', False, 'whether to enable XLA auto-jit compilation')
    ...
    if need_options_and_metadata:
        ...
        if partitioned_graph_file_prefix and step == -2:
            ...
            for graph_def in run_metadata.partition_graphs:
                device = graph_def.node[0].device.replace(
                    '/', '_').replace(':', '_')
...
# Params directly.
Params = namedtuple('Params', flags.param_specs.keys()
...
class BenchmarkCNN(object):
    ...
    def raw_devices_across_tasks(self, is_local=False):
        ...
        if is_local:
            ...
            return [
                'job:worker/replica:0/task%s/%s:%i' % (t,
                                                       self.params.device, i)
    ...
    def _eval_once(self, saver, summary_writer, target, local_var_init_op_group,
        ...
        with tf.Session(
            ...
            images_per_sec = (self.num_batches *
    ...
    def _benchmark_graph(self, graph_info):
        ...
        with sv.managed_session(
            ...
            if self.params.backbone_model_path is not None:
                self.model.load_backbone_model(
                    sess, self.params.backbone_model_path)
            ...
            if not global_step_watcher:
                ...
                def done_fn(): return local_step >= end_local_step
            else:
        ...
        with updated_graph.as_default():
            ...
            for variable in variables_to_keep:
                ...
                tf.add_to_collection(
                    variables_to_keep[variable], updated_variable)
    ...
    def _build_fetches(self, global_step, all_logits, losses, device_grads,
        ...
        for name, ops in all_accuracy_ops.items():
            ...
            if name.startswith(constants.UNREDUCED_ACCURACY_OP_PREFIX):
                fetches[name[len(
                    constants.UNREDUCED_ACCURACY_OP_PREFIX):]] = ops[0]
            else:
        ...
        for d, device in enumerate(apply_gradient_devices):
            with tf.device(device):
                ...
                learning_rate = tf.identity(
                    learning_rate, name='learning_rate_tensor')
        ...
        with tf.device(self.cpu_device):
            if self.task_index == 0 and self.params.summary_verbosity >= 1:
                ...
                tf.summary.scalar(
                    self.params.loss_type_to_report, average_loss)
    ...
    def add_forward_pass_and_gradients(self,
        ...
        def forward_pass_and_gradients():
            ...
            base_loss = self.model.loss_function(
                input_list, build_network_result)
            ...
            with tf.name_scope('l2_loss'):
                ...
                if rel_device_num == len(self.devices) - 1:
                    ...
                    if custom_l2_loss is not None:
                        ...
                        # tensors.
                        reshaped_params = [tf.reshape(
                            p, (-1,)) for p in fp32_params]
                        l2_loss = tf.nn.l2_loss(
                            tf.concat(reshaped_params, axis=0))
                    else:
                        l2_loss = tf.add_n([tf.nn.l2_loss(v)
                                            for v in fp32_params])
            ...
            grads = tf.gradients(scaled_loss, params,
