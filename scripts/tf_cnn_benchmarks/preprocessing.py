...
class Cifar10ImagePreprocessor(BaseImagePreprocessor):
    ...
    def _distort_image(self, image):
        ...
        if self.summary_verbosity >= 3:
            tf.summary.image('distorted_image',
                             tf.expand_dims(distorted_image, 0))
        return distorted_image
    ...
    def _eval_image(self, image):
        ...
        if self.summary_verbosity >= 3:
            tf.summary.image(
                'cropped.image', tf.expand_dims(distorted_image, 0))
        return distorted_image
...
class LibrispeechPreprocessor(InputPreprocessor):
    ...
    def __init__(self, batch_size, output_shapes, num_splits, dtype, train,
                 **kwargs):
        del kwargs
        super(LibrispeechPreprocessor, self).__init__(
    ...
    def minibatch(self, dataset, subset, use_datasets,
        ...
        with tf.name_scope('batch_processing'):
            ...
            # label lengths
            input_lists = [[None for _ in range(
                self.num_splits)] for _ in range(4)]
            for d in xrange(self.num_splits):
